...
# --- Page Configuration ---
st.set_page_config(
    page_title="QuLab: Lab 49: ML-Enhanced Factor Model ", layout="wide")
...
FIXED_N_STOCKS = 200
FIXED_N_MONTHS = 120
FIXED_MIN_TRAIN_MONTHS = 60
FIXED_REBALANCE_FREQ = 1
FIXED_SEED = 42
...
if st.session_state.page == "Introduction":
    st.markdown(
        f"## Introduction: Enhancing Stock Selection for CFA Charterholders")
    ...
    st.markdown(
        f"* Implement OLS and XGBoost models for cross-sectional return prediction.")
    st.markdown(
        f"* Execute a robust walk-forward backtest with monthly rebalancing.")
    st.markdown(
        f"* Form long-short quintile portfolios based on model predictions.")
    st.markdown(
        f"* Compare model performance using Sharpe ratio, maximum drawdown, return, volatility, hit rate, and turnover.")
    st.markdown(
        f"* Conduct SHAP analysis to interpret the ML model's feature importance.")
    st.markdown(
        f"* Assess the governance implications of deploying a more complex ML model based on its performance lift.")

elif st.session_state.page == "1. Data Generation":
    st.markdown(
        f"## 1. Generating Synthetic Panel Data with Non-Linear Alpha Signals")
    ...
    st.markdown(f"**Fixed Parameters for Reproducibility:**")
    st.markdown(f"- Number of Stocks: `{FIXED_N_STOCKS}`")
    st.markdown(f"- Number of Months: `{FIXED_N_MONTHS}` (10 years)")
    st.markdown(f"- Random Seed: `{FIXED_SEED}`")
    ...
    if st.button("Generate Synthetic Data"):
        ...
        if loaded_data is not None:
            ...
            st.info("✅ Loaded previously saved data for reproducibility.")
        else:
            with st.spinner("Generating data..."):
                df, feature_cols = generate_synthetic_data(
                    FIXED_N_STOCKS, FIXED_N_MONTHS, seed=FIXED_SEED)
                st.session_state.df = df
                st.session_state.feature_cols = feature_cols
                st.session_state.data_generated = True
                st.session_state.backtest_run = False
                ...
                save_data(df, feature_cols)
                st.success("Data Generated and Saved!")

    if st.session_state.data_generated:
        st.markdown(
            f"Panel Data Generated: `{FIXED_N_STOCKS} stocks x {FIXED_N_MONTHS} months = {len(st.session_state.df)} observations`")
        st.markdown(
            f"Number of Features: `{len(st.session_state.feature_cols)}`")
        st.markdown(f"First 5 rows of the generated data:")
        st.dataframe(st.session_state.df.head())
        st.markdown(
            f"Return distribution: mean=`{st.session_state.df['next_month_return'].mean():.4f}`, std=`{st.session_state.df['next_month_return'].std():.4f}`")
    ...
    st.markdown(
        f"## 2. Walk-Forward Backtesting and Long-Short Portfolio Construction")
    ...
    st.markdown(
        f"1. Train the model (either OLS or XGBoost) on historical data up to that point.")
    st.markdown(f"2. Predict returns for the *next* month's stocks.")
    st.markdown(f"3. Form a **long-short quintile portfolio**:")
    st.markdown(
        f"    * Go long the top quintile (20% of stocks) with the highest predicted returns.")
    st.markdown(
        f"    * Go short the bottom quintile (20% of stocks) with the lowest predicted returns.")
    st.markdown(
        r"    * The return for this long-short portfolio at time $t$ ($r_{L/S,t}$) is calculated as the mean return of the top quintile ($Q_5$) minus the mean return of the bottom quintile ($Q_1$):")
    st.markdown(r"""
$$
r_{L/S,t} = \frac{{1}}{{|Q_5|}} \sum_{{i \in Q_5}} r_{{i,t}} - \frac{{1}}{{|Q_1|}} \sum_{{i \in Q_1}} r_{{i,t}}
$$""")
    st.markdown(
        r"where $r_{{i,t}}$ is the actual return of stock $i$ at month $t$.")
    st.markdown(
        f"This approach directly monetizes the models' predictive power by identifying the strongest buys and sells.")

    if not st.session_state.data_generated:
        st.warning(
            "Please generate synthetic data first on the '1. Data Generation' page.")
    else:
        st.subheader("Backtest Parameters")
        st.markdown(f"**Fixed Parameters for Reproducibility:**")
        st.markdown(f"- Minimum Training Months: `{FIXED_MIN_TRAIN_MONTHS}`")
        st.markdown(
            f"- Rebalance Frequency: `{FIXED_REBALANCE_FREQ}` month(s)")

        if st.button("Run Walk-Forward Backtest"):
            # Check if backtest results already exist
            loaded_results = load_backtest_results()
            if loaded_results is not None:
                linear_results, ml_results = loaded_results
                st.info(
                    "✅ Loaded previously saved backtest results for reproducibility.")
            else:
                with st.spinner("Running backtests for OLS and XGBoost... This may take a moment."):
                    ...
                    linear_results = walk_forward_backtest(
                        ...
                        min_train_months=FIXED_MIN_TRAIN_MONTHS,
                        rebalance_freq=FIXED_REBALANCE_FREQ
                    )
                    # Run ML Backtest
                    ...
                    save_backtest_results(linear_results, ml_results)
            ...
            # Check if SHAP data already exists
            loaded_shap = load_shap_data()
            if loaded_shap is not None:
                importance, shap_values, shap_sample_data, ml_final_model_for_shap = loaded_shap
                ...
                shap.summary_plot(
                    shap_values, shap_sample_data, show=False, plot_size=(10, 6))
                ...
                min_train_months_for_shap = FIXED_MIN_TRAIN_MONTHS
                train_all_for_shap = st.session_state.df[st.session_state.df['month']
                                                         < min_train_months_for_shap]
                ...
                if not train_all_for_shap.empty and len(train_all_for_shap) > 1000:
                    ...
                    ml_final_model_for_shap.fit(
                        train_all_for_shap[st.session_state.feature_cols], train_all_for_shap['next_month_return'])

                    explainer = shap.TreeExplainer(ml_final_model_for_shap)
                    shap_sample_data = train_all_for_shap[st.session_state.feature_cols].sample(
                        min(1000, len(train_all_for_shap)), random_state=42)
                    shap_values = explainer.shap_values(shap_sample_data)
                    importance = pd.Series(np.abs(shap_values).mean(
                        axis=0), index=st.session_state.feature_cols).sort_values(ascending=False)
                    ...
                    # Save SHAP data
                    save_shap_data(importance, shap_values,
                                   shap_sample_data, ml_final_model_for_shap)
                    ...
                    st.warning(
                        "Not enough training data for robust SHAP analysis. Skipping SHAP generation.")
                    st.session_state.shap_data_ready = False
        ...
        if st.session_state.backtest_run:
            st.markdown(
                f"Linear (OLS) Strategy: `{len(st.session_state.linear_results)}` months of backtest results generated.")
            st.markdown(
                f"Mean L/S Return (OLS): `{st.session_state.linear_results['ls_return'].mean()*100:.2f}%/month`")
            st.markdown(
                f"ML (XGBoost) Strategy: `{len(st.session_state.ml_results)}` months of backtest results generated.")
            st.markdown(
                f"Mean L/S Return (XGBoost): `{st.session_state.ml_results['ls_return'].mean()*100:.2f}%/month`")
            st.markdown(
                f"Sample of Long-Short Returns (first 5 months of ML strategy):")
            st.dataframe(st.session_state.ml_results[[
                         'month', 'ls_return']].head())
...
$$""")
    st.markdown(
        r"where $\mu_{{L/S}}$ is the monthly mean of long-short returns and $\sigma_{{L/S}}$ is the monthly standard deviation of long-short returns.")
    st.markdown(
        f"* **Maximum Drawdown**: The largest peak-to-trough decline over a specific period.")
    st.markdown(r"""
$$
\text{{Max Drawdown}} = \max_t \left( \frac{{V_{{peak,t}} - V_t}}{{V_{{peak,t}}}} \right)
$$""")
    st.markdown(
        r"where $V_t$ is the portfolio value at time $t$, and $V_{{peak,t}}$ is the maximum portfolio value up to time $t$.")
    st.markdown(
        f"* **Hit Rate**: The percentage of months with positive long-short returns.")
    st.markdown(
        f"* **Total Return**: Cumulative return over the entire backtesting period.")
    ...
    if not st.session_state.backtest_run:
        st.warning(
            "Please run the backtest first on the '2. Backtesting & Portfolio Construction' page.")
    else:
        ...
        (1 + st.session_state.linear_results['ls_return']).cumprod().plot(
            label='Linear (OLS) Strategy', color='blue', ax=ax_cum_ret)
        (1 + st.session_state.ml_results['ls_return']).cumprod().plot(
            label='ML (XGBoost) Strategy', color='green', ax=ax_cum_ret)
    ...
    st.markdown(
        f"## 4. Model Interpretability (SHAP) and Deployment Governance Assessment")
    ...
    st.markdown(
        f"* **Substantial Alpha Lift**: ML complexity is justified, proceed with Tier 2 governance (validation + monitoring).")
    st.markdown(r"""
$$
\text{Sharpe Lift} > 0.3
$$""")
...
$$""")
    ...
    if not st.session_state.backtest_run:
        ...
        st.markdown(
            f"Mean Absolute SHAP Value (higher value means greater importance):")

        if st.session_state.shap_summary_plot:
            st.pyplot(st.session_state.shap_summary_plot)

        st.subheader("Governance Assessment")
        lin_sharpe = st.session_state.comparison_table_raw.loc['Linear (OLS)',
                                                               'sharpe']
        ml_sharpe = st.session_state.comparison_table_raw.loc['ML (XGBoost)',
                                                              'sharpe']
        ...
        st.markdown(
            f"Relative Sharpe Lift: `{(ml_sharpe / max(lin_sharpe, 0.01) - 1) * 100:+.0f}%`")

        if sharpe_lift > 0.3:
            st.markdown(
                f"**VERDICT: Substantial alpha lift. ML complexity JUSTIFIED.**")
            st.markdown(
                f"         Proceed with Tier 2 governance (validation + monitoring).")
        elif sharpe_lift > 0.1:
            st.markdown(
                f"**VERDICT: Moderate alpha lift. ML complexity CONDITIONALLY justified.**")
            st.markdown(
                f"         Requires SHAP review + ongoing performance monitoring.")
        else:
            st.markdown(
                f"**VERDICT: Marginal alpha lift. ML complexity NOT justified.**")
            st.markdown(
                f"         Deploy the simpler linear model (lower governance burden).")

        st.markdown(f"---")
        st.markdown(f"**Practitioner Warning**")
        st.markdown(
            f"Simulated data typically overstates ML's advantage because nonlinear interactions are embedded.")
        st.markdown(
            f"Real-world Sharpe lifts are often in the 0.1-0.3 range and come with hidden costs like higher turnover.")
        st.markdown(
            f"Always validate on out-of-sample real data before drawing deployment conclusions.")
